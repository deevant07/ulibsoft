...
public class CdStfDefaulterList extends JFrame
  {
     ...
     public CdStfDefaulterList()
       {
           super("STAFF  DEFAULTERS  LIST  OF  CDS/FLOPPYS");
         ...
         try
         {
             ...
             cd.removeAllItems();
             //cd.addItem("");
             rs=s.executeQuery("SELECT CD1.LID FROM CDTRANSACTION1 CD1,KEYCONSTRAINTS KEY WHERE CD1.RDATE < (SELECT KEY.TO_HOLD FROM KEYCONSTRAINTS KEY WHERE ID=2) AND KEY.TO_HOLD<SYSDATE");
             while(rs.next())
             {
               ...
               public void actionPerformed(ActionEvent e)
                 {
                     try
                     {
                       ...
                       boolean success= plist.print( JTable.PrintMode.NORMAL,
                                                       new MessageFormat("STAFF CD-DEFAULTERS LIST SEARCH ->"+date.getDate()+"/"+date.getMonth()+"/"+(1900+date.getYear())),
                                                       new MessageFormat("STAFF CD-DEFAULTERS LIST SEARCH ->"+date.getHours()+":"+date.getMinutes()+":"+date.getSeconds()),
                                                       true,
                                                       null,
                                                       true
                                                      );
           ...
           public void actionPerformed(ActionEvent e)
             {
                ...
                {
                   s=con.createStatement();
                   rs=s.executeQuery("SELECT CD.CDCODE,CD.CDNAME,CD.CDVERSION,CD1.IDATE,CD1.RDATE FROM CDDETAILS CD,CDTRANSACTION1 CD1 WHERE CD.CDCODE IN(SELECT CD1.CDCODE FROM CDTRANSACTION1 CD1 WHERE CD1.LID="+"'"+cd.getSelectedItem()+"'"+") AND CD1.CDCODE IN(SELECT CD1.CDCODE FROM CDTRANSACTION1 CD1 WHERE CD1.LID='"+cd.getSelectedItem()+"')");
                   while(rs.next())
                   {
                     getTable(rs);
                   }
  ...
  private void getTable(ResultSet rs )
     ...
     public static void main(String a[])
       {
         CdStfDefaulterList pl = new CdStfDefaulterList();
       }
 }
