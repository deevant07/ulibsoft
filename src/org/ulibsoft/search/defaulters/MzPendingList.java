...
public class MzPendingList extends JFrame
  {
     ...
     public MzPendingList()
       {
         super( "MAGEZINE/JOURNAL PENDINGLIST" );
               ...
               public void actionPerformed(ActionEvent e)
                 {
                     try
                     {
                       ...
                       boolean success= plist.print( JTable.PrintMode.NORMAL,
                                                       new MessageFormat("STUDENT MAGAZINE-DEFAULTERS LIST SEARCH ->"+date.getDate()+"/"+date.getMonth()+"/"+(1900+date.getYear())),
                                                       new MessageFormat("STUDENT MAGAZINE-DEFAULTERS LIST SEARCH ->"+date.getHours()+":"+date.getMinutes()+":"+date.getSeconds()),
                                                       true,
                                                       null,
                                                       true
                                                      );
               ...
               public void actionPerformed(ActionEvent e)
                 {
                     try
                     {
                       ...
                       java.util.Date date=new java.util.Date();
                       boolean success= plist.print( JTable.PrintMode.NORMAL,
                                                       new MessageFormat("STUDENT MAGAZINE-DEFAULTERS LIST SEARCH BY BRANCH ->"+date.getDate()+"/"+date.getMonth()+"/"+(1900+date.getYear())),
                                                       new MessageFormat("STUDENT MAGAZINE-DEFAULTERS LIST SEARCH BY NRANCH->"+date.getHours()+":"+date.getMinutes()+":"+date.getSeconds()),
                                                       true,
                                                       null,
                                                       true
                                                      );
               ...
               public void actionPerformed(ActionEvent e)
                 {
                     try
                     {
                       ...
                       java.util.Date date=new java.util.Date();
                       boolean success= plist.print( JTable.PrintMode.NORMAL,
                                                       new MessageFormat("STUDENT MAGAZINE-DEFAULTERS LIST SEARCH BY YEAR->"+date.getDate()+"/"+date.getMonth()+"/"+(1900+date.getYear())),
                                                       new MessageFormat("STUDENT MAGAZINE-DEFAULTERS LIST SEARCH BY YEAR->"+date.getHours()+":"+date.getMinutes()+":"+date.getSeconds()),
                                                       true,
                                                       null,
                                                       true
                                                      );
               ...
               public void actionPerformed(ActionEvent e)
                 {
                     try
                     {
                       ...
                       java.util.Date date=new java.util.Date();
                       boolean success= plist.print( JTable.PrintMode.NORMAL,
                                                       new MessageFormat("STUDENT MAGAZINE-DEFAULTERS LIST SEARCH BY BRANCH&YEAR->"+date.getDate()+"/"+date.getMonth()+"/"+(1900+date.getYear())),
                                                       new MessageFormat("STUDENT MAGAZINE-DEFAULTERS LIST SEARCH BY BRANCH&YEAR->"+date.getHours()+":"+date.getMinutes()+":"+date.getSeconds()),
                                                       true,
                                                       null,
                                                       true
                                                      );
              ...
              try
                {
                   ...
                   rs = s.executeQuery("SELECT STD.ADNO, STD.SNAME, STD.BRANCH, STD.YEAR,TO_CHAR(MZT.RDATE,'DD-MON-YYYY') RDATE FROM STUDENTDETAILS STD,MZTRANSACTION MZT WHERE STD.ADNO=MZT.ADNO AND  MZT.RDATE="+"'"+df.getText ()+"'");
                   if(rs.next())
                    {
                      getTable(rs);
                    }
          ...
          public void actionPerformed(ActionEvent e)
            {
              try
                {
                   s=con.createStatement ();
                   rs=s.executeQuery("SELECT STD.ADNO, STD.SNAME, STD.BRANCH, STD.YEAR,TO_CHAR(MZT.RDATE,'DD-MON-YYYY') RDATE FROM STUDENTDETAILS STD,MZTRANSACTION MZT WHERE STD.ADNO=MZT.ADNO AND RDATE < (SELECT SYSDATE FROM DUAL)");
                   if(rs.next())
                    {
                       getTable(rs);
                       pl.setVisible(false);
          ...
          public void actionPerformed(ActionEvent ae)
            {
              try
                {
                  s=con.createStatement ();
                  rs=s.executeQuery ("SELECT  STD.ADNO, STD.SNAME, STD.YEAR,TO_CHAR(MZT.IDATE,'DD-MON-YYYY') IDATE,TO_CHAR(MZT.RDATE,'DD-MON-YYYY') RDATE FROM STUDENTDETAILS STD,MZTRANSACTION MZT WHERE STD.ADNO=MZT.ADNO AND STD.BRANCH="+"'"+bh.getSelectedItem ()+"'"+"AND MZT.RDATE<(SELECT TO_CHAR(SYSDATE,'DD-MON-YYYY') FROM DUAL)");
                  if(rs.next ())
                   {
                     getTable(rs);
                     g1.setVisible(false);
          ...
          public void actionPerformed(ActionEvent ae)
            {
              try
                {
                  s=con.createStatement ();
                  rs=s.executeQuery ("SELECT  STD.ADNO, STD.SNAME, STD.BRANCH,TO_CHAR(MZT.IDATE,'DD-MON-YYYY') IDATE,TO_CHAR(MZT.RDATE,'DD-MON-YYYY') RDATE FROM STUDENTDETAILS STD ,MZTRANSACTION MZT WHERE STD.ADNO=MZT.ADNO AND STD.YEAR="+"'"+yr.getSelectedItem()+"'"+"AND MZT.RDATE<(SELECT TO_CHAR(SYSDATE,'DD-MON-YYYY') FROM DUAL)");
                    if(rs.next ())
                     {
                       getTable(rs);
                       g2.setVisible(false);
          ...
          public void actionPerformed(ActionEvent ae)
            {
              try
                {
                  s=con.createStatement ();
                  rs=s.executeQuery ("SELECT  STD.ADNO, STD.SNAME, TO_CHAR(MZT.IDATE,'DD-MON-YYYY') IDATE,TO_CHAR(MZT.RDATE,'DD-MON-YYYY') RDATE FROM STUDENTDETAILS STD ,MZTRANSACTION MZT WHERE STD.ADNO=MZT.ADNO AND STD.BRANCH="+"'"+bh.getSelectedItem ()+"'" +" AND STD.YEAR="+ "'"+yr.getSelectedItem ()+"'"+"AND MZT.RDATE<(SELECT TO_CHAR(SYSDATE,'DD-MON-YYYY') FROM DUAL)");
                    if(rs.next ())
                     {
                       getTable(rs);
  ...
  private class adapter7 extends WindowAdapter
    {
  ...
  private void getTable(ResultSet rs )
     ...
     public static void main(String a[])
       {
         MzPendingList pl = new MzPendingList();
       }
 }
