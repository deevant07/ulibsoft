...
public class MzStfDefaulterList extends JFrame
  {
     ...
     public MzStfDefaulterList()
       {
           super(" STAFF  DEFAULTERS  LIST OF  MAGAZINES");
          ...
          try
         {
             ...
             cd.removeAllItems();
             rs=s.executeQuery("SELECT MZ1.LID FROM MZTRANSACTION1 MZ1,KEYCONSTRAINTS KEY WHERE MZ1.RDATE < (SELECT KEY.TO_HOLD FROM KEYCONSTRAINTS KEY WHERE ID=2) AND KEY.TO_HOLD<SYSDATE");
             while(rs.next())
             {
               ...
               public void actionPerformed(ActionEvent e)
                 {
                     try
                     {
                       ...
                       boolean success= plist.print( JTable.PrintMode.NORMAL,
                                                       new MessageFormat("STAFF MAGAZINE-DEFAULTERS LIST SEARCH ->"+date.getDate()+"/"+date.getMonth()+"/"+(1900+date.getYear())),
                                                       new MessageFormat("STAFF MAGAZINE-DEFAULTERS LIST SEARCH ->"+date.getHours()+":"+date.getMinutes()+":"+date.getSeconds()),
                                                       true,
                                                       null,
                                                       true
                                                      );
           ...
           public void actionPerformed(ActionEvent e)
             {
                ...
                {
                   s=con.createStatement();
                   rs=s.executeQuery("SELECT MZ.ACCESSNO,MZ.MZNAME,MZ.VOLUME,MZ1.IDATE,MZ1.RDATE FROM MZDETAILS MZ,MZTRANSACTION1 MZ1 WHERE MZ.ACCESSNO IN(SELECT MZ1.MZCODE FROM MZTRANSACTION1 MZ1 WHERE MZ1.LID="+"'"+cd.getSelectedItem()+"'"+") AND MZ1.MZCODE IN(SELECT MZ1.MZCODE FROM MZTRANSACTION1 MZ1 WHERE MZ1.LID='"+cd.getSelectedItem()+"')");
                   while(rs.next())
                   {
                     getTable(rs);
                   }
   ...
   private void getTable(ResultSet rs )
     ...
     public static void main(String a[])
       {
         MzStfDefaulterList pl = new MzStfDefaulterList();
       }
 }
