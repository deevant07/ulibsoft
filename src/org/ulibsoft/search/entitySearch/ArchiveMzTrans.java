...
public class ArchiveMzTrans extends JFrame
  {
     ...
     private JButton ins, can, next,print ;
     private Container c;
     ...
     private void createComponents()
       {
           ...
           but0 = new JLabel( "    SEARCH  ARCCHIVE  RECORDS" ) ;
           ...
           but7 = new JButton("MAGAZINE  ACCESS  NUMBER");
           but7.setBackground(new Color(0,0,50));
           but7.setForeground(Color.cyan);
           but7.setBorder(new BevelBorder(0));
           cmppn.add ( but7, new AbsoluteConstraints( 10, 80+10, 180-15, 30 ) );
         ...
         try
           {
            ...
            /*  int j =s.executeUpdate(" CREATE VIEW STUDENT_HISTORY3 AS SELECT  STD.ADNO, STD.SNAME,"+
                                     " STD.BRANCH, CD.ACCESSNO, CD.MZNAME, CD.VOLUME, BD.IDATE,"+
                                     " BD.RDATE FROM STUDENTDETAILS STD, MZDETAILS CD, BOOKDETAILS BD"+
                                     " WHERE  STD.ADNO = BD.ADNO AND CD.ACCESSNO = BD.MZCODE " );
              con.commit();
              //JOptionPane.showMessageDialog(null,"created1");
              int k =s.executeUpdate(" CREATE VIEW STAFF_HISTORY3 AS SELECT  STD.LID, STD.LNAME,"+
                                     " STD.DEPT, CD.ACCESSNO, CD.MZNAME, CD.VOLUME, BD.IDATE,"+
                                     " BD.RDATE FROM STAFF STD, MZDETAILS CD, BOOKDETAILS BD"+
                                     " WHERE  STD.LID = BD.LID AND CD.ACCESSNO = BD.MZCODE " );
              con.commit();*/
              //JOptionPane.showMessageDialog(null,"created2");
             ...
             int y = s.executeUpdate("INSERT INTO TEMPTRANS (ADNO,SNAME,BRANCH,CODE,MNAME,VOLUME,IDATE,RDATE)  SELECT * FROM STUDENT_HISTORY3");
             con.commit();
             int z = s.executeUpdate("INSERT INTO TEMPTRANS (LID,LNAME,DEPT,CODE,MNAME,VOLUME,IDATE,RDATE)  SELECT * FROM STAFF_HISTORY3");
             con.commit();
     ...
     private void inputConstraints()
       {
           ...
           //INPUT CONSTRAINTS FOR STUDENT DETAILS
            ...
            mzno = new JLabel ( "MAG_ACS_NO" );
            mzno.setForeground ( new Color ( 120, 120, 153 ) );
            mzno.setHorizontalAlignment ( SwingConstants.RIGHT  );
            cmppn1.add ( mzno, new AbsoluteConstraints( 12, 51+5+30-15-10 ) );

            mzno1 = new JTextField ( );
            //mzno1.setEditable(false);
            mzno1.setBorder ( new MatteBorder ( 1, 1, 1, 1,new Color(0,0,40) ) );
            mzno1.setForeground ( new Color ( 255, 0, 153 ) );
            mzno1.setCaretColor ( new Color ( 0, 204, 102 ) );
            cmppn1.add ( mzno1, new AbsoluteConstraints( 115+16-20,50+5+30-15-10,135,20) );
            ...
            cmppn1.add ( batch1, new AbsoluteConstraints( 115+16-20, 111+5+29-15-10,192-7, 20 ) );
            //cmppn1.add ( batch1, new AbsoluteConstraints( 265, 110+5,110, 20 ) );
            ...
            cmppn1.add ( to1, new AbsoluteConstraints( 395, 111+5+29-15-10,135, 20 ) );
     ...
     private void componentListener()
       {
          ...
          but7.addActionListener(new ActionListener()
          {
            public void actionPerformed(ActionEvent e)
            {
               if(columnsParam == null)
                 {
                      columnsParam = "CODE, MNAME, VOLUME";
                 }
                 else
                 {
                      columnsParam = columnsParam+","+"CODE, MNAME, VOLUME";
                 }
            }
          }
          );
          but8.addActionListener(new ActionListener()
          {
            public void actionPerformed(ActionEvent e)
            {
               if(columnsParam == null)
                 {
                      columnsParam = "  TO_CHAR(IDATE,'DD-MON-YYYY') IDATE";
                 }
                 else
                 {
                      columnsParam = columnsParam+","+"  TO_CHAR(IDATE,'DD-MON-YYYY') IDATE";
                 }
            }
          }
          );
          but9.addActionListener(new ActionListener()
          {
            public void actionPerformed(ActionEvent e)
            {
               if(columnsParam == null)
                 {
                      columnsParam = "  TO_CHAR(RDATE,'DD-MON-YYYY') RDATE";
                 }
                 else
                 {
                      columnsParam = columnsParam+","+"  TO_CHAR(RDATE,'DD-MON-YYYY') RDATE";
                 }
            }
          }
          );
                ...
                public void itemStateChanged(ItemEvent e)
                   {
                      if(count3<1)
                        {
                          item3++;
                          if( mzno1.getText()==null || mzno1.getText().equals("") ) { }
                          else
                           {
                              BKNO = mzno1.getText().toUpperCase();
                     ...
                     try
                       {
                        // JOptionPane.showMessageDialog(null,constraints);
                          ...
                          if(columnsParam !=null)
                            ...
                            {
                               if(constraints.equals(""))
                                 {  //JOptionPane.showMessageDialog(null,"xhxghdxfg");
                                    rs=s.executeQuery("SELECT ADNO,SNAME,BRANCH,LID,LNAME,DEPT,CODE ACCESSNO,MNAME,VOLUME,TO_CHAR(IDATE,'DD-MON-YYYY') ISSUED_DATE,TO_CHAR(RDATE,'DD-MON-YYYY') RECIEVED_DATE  FROM TEMPTRANS ");
                                    if(rs.next())
                                      {
                                         getTable(rs);
                                      }
                                 ...
                                 {
                                    rs=s.executeQuery("SELECT ADNO,SNAME,BRANCH,LID,LNAME,DEPT,CODE ACCESSNO,MNAME,VOLUME,TO_CHAR(IDATE,'DD-MON-YYYY') ISSUED_DATE,TO_CHAR(RDATE,'DD-MON-YYYY') RECIEVED_DATE  FROM TEMPTRANS  WHERE "+constraints );
                                    if(rs.next())
                                      {
                                         getTable(rs);
                                      }
               ...
               public void actionPerformed(ActionEvent e)
                 {
                     try
                     {
                       ...
                       boolean success= sttable.print( JTable.PrintMode.NORMAL,
                                                       new MessageFormat("ARCHIVE MAGAZINE'S TRANSACTIONS ->"+date.getDate()+"/"+date.getMonth()+"/"+(1900+date.getYear())),
                                                       new MessageFormat("ARCHIVE MAGAZINE'S TRANSACTIONS ->"+date.getHours()+":"+date.getMinutes()+":"+date.getSeconds()),
                                                       true,
                                                       null,
                                                       true
                                                      );
          ...
          can.addActionListener(new ActionListener()
            {
               public void actionPerformed(ActionEvent e)
               {
                  try
                    {
                       s = con.createStatement();
                       int d1 = s.executeUpdate("DROP TABLE TEMPTRANS");
                      // int d2 = s.executeUpdate("DROP VIEW STUDENT_HISTORY3");
                       //int d3 = s.executeUpdate("DROP VIEW STAFF_HISTORY3");
