...
public class MzCopies extends JFrame
  {
    ...
    private void createComponents()
      {
        ...
        bkpn.setBorder( new TitledBorder ( new EtchedBorder ( Color.cyan, new Color(0,0,40)),
                                            "COPIES FOR A MAGAZINE", 1, 2, c.getFont(), Color.magenta));
           ...
           try
             {
                ...
                rs = s.executeQuery("SELECT DISTINCT(MZNAME) FROM MZDETAILS ORDER BY MZNAME");
                   while(rs.next ())
                     {
            ...
            public void itemStateChanged(ItemEvent e)
              {
                ...
                try
                  {
                    s = con.createStatement();
                    rs = s.executeQuery("SELECT DISTINCT(VOLUME) FROM MZDETAILS WHERE MZNAME="+"'"+cn1.getSelectedItem()+"'"+"ORDER BY VOLUME");
                       while( rs.next () )
                         {
                ...
                try
                  {
                    s = con.createStatement();
                        ...
                        rs = s.executeQuery("SELECT COUNT(MZNAME) FROM MZDETAILS WHERE  MZNAME="+"'"+cn1.getSelectedItem()+"'"+"AND  VOLUME="+"'"+vrs1.getSelectedItem()+"'");
                           if( rs.next ())
                             {
                        ...
                        rs = s.executeQuery("SELECT COUNT(MZ.MZNAME) FROM MZDETAILS MZ WHERE MZ.ACCESSNO NOT IN(SELECT MZT.MZCODE FROM MZTRANSACTION MZT WHERE MZT.MZCODE=MZ.ACCESSNO) AND MZ.ACCESSNO NOT IN(SELECT MZT1.MZCODE FROM MZTRANSACTION1 MZT1 WHERE MZT1.MZCODE=MZ.ACCESSNO) AND MZ.MZNAME="+"'"+cn1.getSelectedItem()+"'"+"AND MZ.VOLUME="+"'"+vrs1.getSelectedItem()+"'" );
                           if( rs.next() )
                             {
                        ...
                        rs = s.executeQuery("SELECT COUNT(MZ.MZNAME) FROM MZDETAILS MZ,MZTRANSACTION MZT WHERE MZ.ACCESSNO=MZT.MZCODE AND MZ.MZNAME="+"'"+cn1.getSelectedItem() +"'"+"AND MZ.VOLUME="+"'"+vrs1.getSelectedItem()+"'" );
                           if( rs.next() )
                             {
                                s1=rs.getString(1);
                             }
                        rs1 = s.executeQuery("SELECT COUNT(MZ.MZNAME) FROM MZDETAILS MZ,MZTRANSACTION1 MZT1 WHERE MZ.ACCESSNO=MZT1.MZCODE AND MZ.MZNAME="+"'"+cn1.getSelectedItem()+"'"+"AND MZ.VOLUME="+"'"+vrs1.getSelectedItem()+"'" );
                            if(rs1.next())
                              {
                                s2=rs1.getString(1);
                              }
