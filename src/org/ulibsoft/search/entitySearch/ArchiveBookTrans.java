...
public class ArchiveBookTrans extends JFrame
  {
     ...
     private void createComponents()
       {
           ...
           but5 = new JButton("BOOK  ACCESS  NUMBER");
         ...
         try
           {
             ...
             int y = s.executeUpdate("INSERT INTO TEMPTRANS (ADNO,SNAME,BRANCH,CODE,BNAME,AUTHOR,IDATE,RDATE)  SELECT * FROM STUDENT_HISTORY");
             con.commit();
             int z = s.executeUpdate("INSERT INTO TEMPTRANS (LID,LNAME,DEPT,CODE,BNAME,AUTHOR,IDATE,RDATE)  SELECT * FROM STAFF_HISTORY");
             con.commit();
     ...
     private void inputConstraints()
       {
           ...
           //INPUT CONSTRAINTS FOR STUDENT DETAILS
            ...
            bkno = new JLabel ( "BOOK_ACS_NO" );
            bkno.setForeground ( new Color ( 120, 120, 153 ) );
            cmppn1.add ( bkno, new AbsoluteConstraints( 12, 51+5+30-15-10 ) );
    ...
    private void componentListener()
       {
          ...
          but5.addActionListener(new ActionListener()
          {
            public void actionPerformed(ActionEvent e)
            {
               if(columnsParam == null)
                 {
                      columnsParam = " CODE, AUTHOR, BNAME";
                 }
                 else
                 {
                      columnsParam = columnsParam+","+" CODE, AUTHOR, BNAME";
                 }
            }
          }
          );
                     ...
                     try
                       {
                          ...
                          if(columnsParam !=null)
                          ...
                          else
                            {
                               if(constraints.equals(""))
                                 {
                                    rs=s.executeQuery("SELECT ADNO,SNAME,BRANCH,LID,LNAME,DEPT,CODE ACCESSNO,BNAME,AUTHOR,TO_CHAR(IDATE,'DD-MON-YYYY') ISSUED_DATE,TO_CHAR(RDATE,'DD-MON-YYYY') RECIEVED_DATE  FROM TEMPTRANS  ");
                                    if(rs.next())
                                      {
                                         getTable(rs);
                                      }
                                 ...
                                 {
                                    JOptionPane.showMessageDialog(null,"1234"+constraints);
                                    rs=s.executeQuery("SELECT ADNO,SNAME,BRANCH,LID,LNAME,DEPT,CODE ACCESSNO,BNAME,AUTHOR,TO_CHAR(IDATE,'DD-MON-YYYY') ISSUED_DATE,TO_CHAR(RDATE,'DD-MON-YYYY') RECIEVED_DATE  FROM TEMPTRANS WHERE "+constraints );
                                    if(rs.next())
                                      {
                                         getTable(rs);
                                      }
               ...
               public void actionPerformed(ActionEvent e)
                 {
                     try
                     {
                       ...
                       boolean success= sttable.print( JTable.PrintMode.NORMAL,
                                                       new MessageFormat("ARCHIVE BOOK'S TRANSACTIONS ->"+date.getDate()+"/"+date.getMonth()+"/"+(1900+date.getYear())),
                                                       new MessageFormat("ARCHIVE BOOK'S TRANSACTIONS ->"+date.getHours()+":"+date.getMinutes()+":"+date.getSeconds()),
                                                       true,
                                                       null,
                                                       true
                                                      );
