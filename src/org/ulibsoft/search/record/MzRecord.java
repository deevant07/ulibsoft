...
public class MzRecord extends JFrame
  {
      private JLabel code;
      private int count=0;
     ...
     public MzRecord()
      ...
      private void createComponents()
      {
         ...
         c.add(p1,new AbsoluteConstraints(90,215+30,600,300));
           ...
           but0 = new JLabel( "         SEARCH  MAGAZINE  RECORD" ) ;
                 ...
                 try
                    {
                      ...
                      rs = s.executeQuery("SELECT STD.ADNO,STD.SNAME,STD.BRANCH,STD.YEAR,TO_CHAR(BD.IDATE,'DD-MON-YYYY') IDATE,TO_CHAR(BD.RDATE,'DD-MON-YYYY') RDATE FROM  STUDENTDETAILS STD,BOOKDETAILS BD WHERE STD.ADNO=BD.ADNO AND BD.MZCODE IN(SELECT MZ.ACCESSNO FROM MZDETAILS MZ WHERE MZ.ACCESSNO ="+"'"+cd.getText()+"')" );
                         if( rs.next () )
                           {
                              std.setEnabled(true);
                              count++;
                       ...
                       rs = s.executeQuery ("SELECT STF.LID,STF.LNAME,STF.DEPT,TO_CHAR(BD.IDATE,'DD-MON-YYYY') IDATE,TO_CHAR(BD.RDATE,'DD-MON-YYYY') RDATE FROM STAFF STF,BOOKDETAILS BD WHERE STF.LID=BD.LID AND BD.MZCODE IN(SELECT MZ.ACCESSNO FROM MZDETAILS MZ WHERE MZ.ACCESSNO="+"'"+cd.getText()+"')" );
                         if( rs.next() )
                           {
                              stf.setEnabled(true);
                              count++;
        ...
        stf.addActionListener(new ActionListener()
         {
             public void actionPerformed(ActionEvent e)
             {
               ...
               {
                 s=con.createStatement();
                 rs = s.executeQuery ("SELECT STF.LID, STF.LNAME, STF.DEPT, TO_CHAR(BD.IDATE,'DD-MON-YYYY') IDATE, TO_CHAR(BD.RDATE,'DD-MON-YYYY') RDATE  FROM BOOKDETAILS BD,STAFF STF WHERE BD.MZCODE="+"'"+cd.getText()+"'"+"AND BD.VALUE='3'"+"AND STF.LID=BD.LID");
                   while(rs.next())
                   {
                       getTable2(rs);
                   }
               ...
               public void actionPerformed(ActionEvent e)
                 {
                     try
                     {
                       ...
                       boolean success= bookdetails.print( JTable.PrintMode.NORMAL,
                                                       new MessageFormat("MAGAZINE-RECORD'S SEARCH ->"+date.getDate()+"/"+date.getMonth()+"/"+(1900+date.getYear())),
                                                       new MessageFormat("MAGAZINE-RECORD'S SEARCH ->"+date.getHours()+":"+date.getMinutes()+":"+date.getSeconds()),
                                                       true,
                                                       null,
                                                       true
                                                      );
