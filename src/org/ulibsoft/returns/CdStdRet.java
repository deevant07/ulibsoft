...
public class CdStdRet extends JFrame
  {
    ...
    private int difference, fineCount = 1;
    private String  temp;

    private static String PATH;
    private int PERIOD,NO_REN,NO_OF_REN,FINE_CD;
    ...
    public CdStdRet()
      {
        ...
        try
         ...
         {
          s=con.createStatement();
          rs=s.executeQuery("SELECT IMAGE_PATH,RENEIVAL_PERIOD_CD,NO_OF_RENEIVALS_CD,FINE_CD FROM KEYCONSTRAINTS WHERE ID=1");
          while(rs.next())
          {
           PATH=rs.getString(1);
           PERIOD=rs.getInt(2);
    ...
    private void createComponents()
      {
         ...
         rtpn.setBorder( new TitledBorder ( new EtchedBorder ( Color.cyan, new Color(0,0,40)),
                                            "RETRIVE -- CDS", 1, 2, c.getFont(), Color.magenta));
         ...
         code = new JLabel ( "CD-CODE " );
         code.setForeground ( new Color ( 120, 120, 153 ) );
         rtpn.add ( code, new AbsoluteConstraints (56, 23 ) );
         ...
         rtpn.add (cdname, new AbsoluteConstraints( 56, 50));
         ...
         rtpn.add ( nm, new AbsoluteConstraints ( 120, 48, 125, 18 ) );

         cdvrsion = new JLabel( "CD-VERSION" );
         cdvrsion.setForeground ( new Color ( 120, 120, 153 ) );
         rtpn.add ( cdvrsion, new AbsoluteConstraints ( 37, 77 ) );
         ...
         rtpn.add ( vrs, new AbsoluteConstraints ( 120, 76, 125, 18 ) );
         ...
         rtpn.add ( ano, new AbsoluteConstraints ( 490, 20, 125, 20 ) );
         ...
         rtpn.add ( sn, new AbsoluteConstraints ( 490, 48, 125, 18 ) );
         ...
         rtpn.add ( sb, new AbsoluteConstraints ( 490, 76, 125, 18 ) );

         syear = new JLabel( "SEMESTER" );
         syear.setForeground ( new Color ( 120, 120, 153 ) );
         rtpn.add ( syear, new AbsoluteConstraints ( 420-1, 103 ) );
         ...
         rtpn.add ( sy, new AbsoluteConstraints ( 490, 102, 125, 18 ) );
         ...
         rtpn.add ( fine, new AbsoluteConstraints ( 457, 131 ) );
         ...
         p1.setBorder( new TitledBorder ( new EtchedBorder ( Color.cyan, new Color(0,0,40)),
                                            "CD -- DATABASE", 1, 2,c.getFont(), Color.magenta));
         ...
         try
             {
                fout=new FileOutputStream("c:\\windows\\report33.html");
             }catch(Exception se)
             {
             }
            fp="<HTML>";
            ...
            fp+="<BR><TR BGCOLOR="+"#AADD00"+">";
             try
             {
                fout1=new FileOutputStream("c:\\windows\\report34.html");
             }catch(Exception se)
             {
             }
            ...
            public void actionPerformed( ActionEvent e )
              {
                ...
                try
                  {
                     s = con.createStatement ();
                     rs = s.executeQuery("SELECT STD.ADNO, STD.SNAME, STD.BRANCH, STD.YEAR, CD.CDNAME, CD.CDVERSION, CD.CDCODE, TO_CHAR(CDT.IDATE,'DD-MON-YYYY'), TO_CHAR(CDT.RDATE,'DD-MON-YYYY') FROM CDTRANSACTION CDT,STUDENTDETAILS STD,CDDETAILS CD WHERE CDT.CDCODE=CD.CDCODE AND CD.CDCODE="+"'"+ cd.getText()+"'"+"AND STD.ADNO=CDT.ADNO" );
                       if( rs.next() )
                        {
                          ...
                          nm.setText( rs.getString(5) ) ;
                          vrs.setText ( rs.getString(6) ) ;
                           ...
                           if(d >= 0)
                            {                  //INSERTION
                             ...
                             /*  int r = s.executeUpdate(" INSERT INTO CDLIBRARY ( CDNAME, CDVERSION ,CDCODE) VALUES ( "+"'"+nm.getText()+"',"
                               ...
                               int as=s.executeUpdate("UPDATE CDTRANSACTION SET RDATE=TO_CHAR(SYSDATE,'DD-MON-YYYY') WHERE CDCODE="+"'"+cd.getText()+"'"+"AND IDATE="+"'"+i1.getText()+"'"+"AND ADNO="+"'"+ano.getText()+"'");
                               con.commit();
                               int g=s.executeUpdate("UPDATE BOOKDETAILS SET RDATE=TO_CHAR(SYSDATE,'DD-MON-YYYY') WHERE CDCODE="+"'"+cd.getText()+"'"+"AND IDATE="+"'"+i1.getText()+"'"+"AND ADNO="+"'"+ano.getText()+"'");
                               con.commit();

                                                      //DELETION
                               int x = s.executeUpdate("DELETE FROM CDTRANSACTION WHERE CDCODE ="+"'"+cd.getText()+"'" );
                               con.commit();
             ...
             public void actionPerformed(ActionEvent e)
               {
                 ...
                 try
                   {
                      s = con.createStatement();
                      //int w = s.executeUpdate("UPDATE CDTRANSACTION SET RDATE="+"'"+r1.getText()+"'"+"WHERE CDCODE="+"'"+cd.getText()+"'");
                      rs=s.executeQuery("SELECT NO_OF_REN  FROM CDTRANSACTION WHERE CDCODE="+"'"+cd.getText()+"'");
                       while(rs.next())
                       {
                       ...
                       {     NO_OF_REN=Integer.parseInt(no_of_ren);
                          if(NO_OF_REN < NO_REN)
                          {
                           int w = s.executeUpdate("UPDATE CDTRANSACTION SET RDATE=(SELECT RDATE+"+PERIOD+ " FROM CDTRANSACTION  WHERE CDCODE="+"'"+cd.getText()+"') WHERE CDCODE="+"'"+cd.getText()+"'");
                           int ren1=s.executeUpdate("UPDATE CDTRANSACTION SET NO_OF_REN ='"+(NO_OF_REN + 1)+"' WHERE CDCODE="+"'"+cd.getText()+"'");
                           con.commit();
                           getTable1();
                           ren.setEnabled(false);
                          }
                       ...
                       {
                           int w = s.executeUpdate("UPDATE CDTRANSACTION SET RDATE=(SELECT RDATE+"+PERIOD+ " FROM CDTRANSACTION  WHERE CDCODE="+"'"+cd.getText()+"') WHERE CDCODE="+"'"+cd.getText()+"'");
                           int ren2=s.executeUpdate("UPDATE CDTRANSACTION SET NO_OF_REN ='1' WHERE CDCODE="+"'"+cd.getText()+"'");
                           con.commit();
                           getTable1();
                           ren.setEnabled(false);
                       }
                       rs=s.executeQuery("SELECT TO_CHAR(RDATE,'DD-MON-YYYY') FROM CDTRANSACTION WHERE CDCODE="+"'"+cd.getText()+"'");
                       rs.next();
                       ...
                       if( rs.next() )
                         {
                           ...
                           if( k < 0 )
                               ...
                               if(rs.next())
                                {
                                   getTable2(rs);
                                                ...
                                                //DELETION
                               int AS=s.executeUpdate("UPDATE CDTRANSACTION SET RDATE=TO_CHAR(SYSDATE,'DD-MON-YYYY') WHERE CDCODE="+"'"+cd.getText()+"'"+"AND IDATE="+"'"+i1.getText()+"'");
                               con.commit();
                            ...
                            {
                              JOptionPane.showMessageDialog(null,"EMPTY PENDINGLIST !...","Message",JOptionPane.INFORMATION_MESSAGE);
                            }
                            rs=s.executeQuery("SELECT ISSUED FROM CDDETAILS WHERE CDCODE="+"'"+cd.getText() +"'");
                               while(rs.next())
                               {
                               ...
                               if( confirm == true )
                                {
                                   try
                                     {
                                        s = con.createStatement();
                                        int wq=s.executeUpdate("UPDATE BOOKDETAILS SET FINEAMOUNT="+"'"+f1.getText()+"'"+"WHERE CDCODE="+"'"+cd.getText()+"'"+"AND ADNO="+"'"+ano.getText()+"'"+"AND RDATE="+"'"+r1.getText()+"'");
                                        con.commit();
                                        int wq1=s.executeUpdate("UPDATE BOOKDETAILS SET FINE_PAID="+"'"+f1.getText()+"'"+"WHERE CDCODE="+"'"+cd.getText()+"'"+"AND ADNO="+"'"+ano.getText()+"'"+"AND RDATE="+"'"+r1.getText()+"'");
                                        con.commit();
                                        f1.setText(String.valueOf(0));
                                        int Qwq=s.executeUpdate("UPDATE BOOKDETAILS SET REASON="+"'"+fineArea.getText()+"'"+"WHERE CDCODE="+"'"+cd.getText()+"'"+"AND ADNO="+"'"+ano.getText()+"'"+"AND RDATE="+"'"+r1.getText()+"'");
               ...
               public void actionPerformed(ActionEvent e)
                 {
                     ...
                     try
                       {
                          rt.exec("C:\\Program Files\\Internet Explorer\\IEXPLORE.EXE c:\\windows\\report33.html");
                       }
                                   ...
                                   try
                                     {
                                        s = con.createStatement();
                                        int wq=s.executeUpdate("UPDATE BOOKDETAILS SET FINEAMOUNT="+"'"+f1.getText()+"'"+"WHERE CDCODE="+"'"+cd.getText()+"'"+"AND ADNO="+"'"+ano.getText()+"'"+"AND RDATE="+"'"+r1.getText()+"'");
                                        int wqrr=s.executeUpdate("UPDATE BOOKDETAILS SET FINE_PAID="+"'"+amt1.getText()+"'"+"WHERE CDCODE="+"'"+cd.getText()+"'"+"AND ADNO="+"'"+ano.getText()+"'"+"AND RDATE="+"'"+r1.getText()+"'");
                                        //f1.setText(String.valueOf(0));
                                        int Qwq=s.executeUpdate("UPDATE BOOKDETAILS SET REASON="+"'"+fineArea.getText()+"'"+"WHERE CDCODE="+"'"+cd.getText()+"'"+"AND ADNO="+"'"+ano.getText()+"'"+"AND RDATE="+"'"+r1.getText()+"'");
               ...
               rest1.addActionListener(new ActionListener()
            {
               public void actionPerformed(ActionEvent e)
                 {   fineValue1=Integer.parseInt(f1.getText());
                     ...
                     try
                       {
                          rt.exec("C:\\Program Files\\Internet Explorer\\IEXPLORE.EXE c:\\windows\\report34.html");
                       }
                     catch(IOException ex)
                       {
                       }
          ...
          if( difference > 0 )
               {
                  for( int i =1; i <= difference ; i++ )
                        {
                           rs2 = s.executeQuery( " SELECT TO_CHAR(RDATE+"+i+",'DAY') FROM CDTRANSACTION " );
                               if( rs2.next() )
                                    {
